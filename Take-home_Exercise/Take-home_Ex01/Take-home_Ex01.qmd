...
#| fig-height: 12
p1<-ggplot(data=df) +
  geom_mosaic(aes(x = product(Heart_Attack_Occurrence), fill=Blood_Pressure))+
...
p1<-p1+geom_label(data = ggplot_build(p1)$data[[1]] %>% 
                group_by(PANEL,x__Heart_Attack_Occurrence) %>%
                mutate(pct = .wt/sum(.wt)*100), 
              aes(x = (xmin+xmax)/2, y = (ymin+ymax)/2, label=round(pct, 2)))
...
#Create contingency table
subset_df<-df[df$Age_Groups=='Young',]
contingency_table <- table(subset_df$Blood_Pressure, subset_df$Heart_Attack_Occurrence)
...
#Create contingency table
subset_df<-df[df$Age_Groups=='Young',]
contingency_table <- table(subset_df$Cholesterol_Category, subset_df$Heart_Attack_Occurrence)
