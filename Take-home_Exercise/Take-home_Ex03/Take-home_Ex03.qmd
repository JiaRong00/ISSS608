...
-   enable users to choose the type of statistical approach: "parametric", "nonparametric", "robust" and "bayes".
-   enable users to filter by either a categorical variable or a range of a continuous variable selected for the scatter plot, enabling the visualization of more complex relationships beyond simple linear trends. Grouping by a categorical variable can reveal additional insights.

#### **5.2 Bivariate Analysis: Continuous & Categorical Variables**
In this section, we will do confirmatory analyses for the findings observed for EDA (section 4).
...
-   enable users to choose the p.adjust.method
-   may be usedul to show a table displaying all the statistic results
