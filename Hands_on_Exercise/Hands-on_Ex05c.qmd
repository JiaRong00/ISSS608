...
In this exercise, we will be using the  *World Happines 2018 report* data and the following packages: seriation, heatmaply, dendextend and tidyverse.
...
First, we need to change the rows by country name instead of row number by using the code chunk below.

```{r}
row.names(wh) <- wh$Country
```

Next, we transform the data frame into a matrix using the code chunk below.
...
In this sub-section, we will plot a heatmap by using `heatmap()` of Base Stats using the code chunk below.
...
We can normalize this matrix using the `scale` argument. The code chunk below normalises the matrix column-wise.
...
### **References**
[Heatmap for Visualising and Analysing Multivariate Data](https://r4va.netlify.app/chap14)